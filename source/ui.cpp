...
namespace dbk {
        ...
        if (const Button *activated_button = this->GetActivatedButton(); activated_button != nullptr) {
            switch (activated_button->id) {
                ...
                case ContinueButtonId:
                    ...
                    /* Auto-select Preserve Settings (skip ChooseResetMenu). */
                    g_reset_to_factory = false;

                    /* Check if exfat is supported and auto-select FAT32+exFAT (skip ChooseExfatMenu). */
                    ...
                    /* Create the next menu with confirmation prompt (skip both ChooseResetMenu and ChooseExfatMenu). */
                    std::shared_ptr<Menu> next_menu = std::make_shared<WarningMenu>(g_current_menu, std::make_shared<InstallUpdateMenu>(g_current_menu), "Ready to begin update installation", "Are you sure you want to proceed?");
    ...
    ChooseExfatMenu::ChooseExfatMenu(std::shared_ptr<Menu> prev_menu) : Menu(prev_menu) {
        ...
        /* Add only the FAT32 + exFAT button (FAT32-only option is hidden). */
        this->AddButton(ExFatButtonId, "Install (FAT32 + exFAT)", x + HorizontalInset, y + TitleGap, button_width, ButtonHeight);

        /* Set the exFAT button as selected by default. */
        this->SetButtonSelected(ExFatButtonId, true);
        
        /* Force exFAT mode by default. */
        g_use_exfat = true;
    }
    ...
    void ChooseExfatMenu::Draw(NVGcontext *vg, u64 ns) {
        ...
        DrawWindow(vg, "Select install mode", x, y, WindowWidth, WindowHeight);
        this->DrawButtons(vg, ns);
    }

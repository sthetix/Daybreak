...
$(SUBFOLDERS):
	$(MAKE) -C $@ $(if $(filter clean,$(MAKECMDGOALS)),clean,all)

RELEASE_STAGING := $(TOPDIR)/.release_staging

.PHONY: release
release: all | $(TOPDIR)/release
	@mkdir -p $(RELEASE_STAGING)/switch/daybreak
	@cp $(TOPDIR)/$(TARGET).nro $(RELEASE_STAGING)/switch/daybreak/
	@cd $(RELEASE_STAGING) && zip -r $(TOPDIR)/release/$(TARGET)-$(APP_VERSION).zip switch/
